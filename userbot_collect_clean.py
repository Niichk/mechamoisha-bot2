...
async def build_random_code_comment() -> str:
    txt = await asyncio.to_thread(
        ...
        "Генерируйте короткие мудрости, как будто вы еврейский раввин, дающий совет о деньгах или женщинах или мойшах или жизни под солнцем"
    )
...
async def build_reply_for_comment(user_text: str) -> str:
    ...
    prompt = (
        ...
        "Твои ответ должен быть анекдотом в стиле про евреев. "
        "Анекдот может быть в формате еврей, и две любых других национальности, (но еврей должен быть и он самый мудрый и хитрый) сидят где-то (заходят куда-то) и (еврей всех перехетрил)... "
